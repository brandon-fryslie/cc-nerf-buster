...
def log(msg: str) -> None:
    print(f"[{datetime.now(UTC).strftime('%H:%M:%S')}] {msg}", file=sys.stderr)
...
def main() -> None:
    ...
    log(f"model: {model}  scope: org={baseline['org']} upstream={baseline['upstream']}")
    log(f"baseline util: 5h={int(baseline['util_5h']*100+1e-9)}%  7d={int(baseline['util_7d']*100+1e-9)}%")
    log(f"initial estimate (input-equiv tokens / 1% tick): 5h≈{est_units_per_tick_5h:,.0f}  7d≈{est_units_per_tick_7d:,.0f}")
    log(f"per-iter command: {cmd_preview} <prompt>")
    ...
    for iter_num in range(next_iter, 10_000):
        ...
        log(
            f"#{n} {size_name} ({wall_ms/1000:.1f}s) "
            f"| Tokens: Input {usage['input']} // Output {usage['output']} "
            f"// Cache Read {usage['cache_read']} // Cache Write {usage['cache_create']} "
            f"| Util: 5h {util_pct_5h}% (+{ticks_now_5h}) // 7d {util_pct_7d}% (+{ticks_now_7d}) "
            f"| To next tick: 5h ~{next_5h:,.0f} // 7d ~{next_7d:,.0f}"
        )

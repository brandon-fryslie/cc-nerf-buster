...
DEFAULT_5H_INPUT_EQUIV_PER_TICK = 550_623.0
DEFAULT_7D_INPUT_EQUIV_PER_TICK = DEFAULT_5H_INPUT_EQUIV_PER_TICK * 5
...
def find_previous_run(data_dir: Path, current_run_dir: Path) -> Path | None:
...
def print_comparison(run_dir: Path, data_dir: Path) -> None:
    cur_path = run_dir / "bounds.json"
    if not cur_path.exists():
        return
    prev_run = find_previous_run(data_dir, run_dir)
    log_raw("")
    log_raw(bold("=== Comparison to previous run ==="))
    if prev_run is None:
        log_raw(dim("  (no previous non-dry-run with bounds.json found)"))
        return
...
def main() -> None:
    ...
    if args.window == "5h":
        log(f"target: {target_5h_ticks} ticks on the 5h window only")
    elif args.window == "7d":
        log(f"target: {target_7d_ticks} ticks on the 7d window only")
    else:
        log(f"target: {target_5h_ticks} ticks on the 5h window, {target_7d_ticks} ticks on the 7d window")
    if args.dry_run:
    ...
    # input-equiv consumed / total ticks crossed, replacing the proxy's
    # initial guess with ground truth as soon as we have any.
    observed_units_5h = 0.0
    observed_ticks_5h = 0
    observed_units_7d = 0.0
    observed_ticks_7d = 0
    ...
    log(
        ...
        f"5h window {int(baseline['util_5h']*100+1e-9)}%, "
        f"7d window {int(baseline['util_7d']*100+1e-9)}%"
    )
    log(
        f"initial estimate of input-equivalent tokens per 1% tick: "
        f"5h window ≈ {est_units_per_tick_5h:,.0f}, "
        f"7d window ≈ {est_units_per_tick_7d:,.0f}"
    )
    ...
    for iter_num in range(next_iter, 10_000):
        ...
        if interrupted:
            ...
            run_report(script_dir, run_dir)
            print_comparison(run_dir, data_dir)
        ...
        used_units_since_5h_tick += iter_units
        used_units_since_7d_tick += iter_units
        observed_units_5h += iter_units
        observed_units_7d += iter_units
        if crossed_5h > 0:
            observed_ticks_5h += crossed_5h
            est_units_per_tick_5h = observed_units_5h / observed_ticks_5h
            used_units_since_5h_tick = max(0.0, used_units_since_5h_tick - est_units_per_tick_5h * crossed_5h)
        if crossed_7d > 0:
            observed_ticks_7d += crossed_7d
            est_units_per_tick_7d = observed_units_7d / observed_ticks_7d
            used_units_since_7d_tick = max(0.0, used_units_since_7d_tick - est_units_per_tick_7d * crossed_7d)
        ...
        ticks_now_5h = tick_delta(snap['util_5h'], baseline['util_5h'])
        ticks_now_7d = tick_delta(snap['util_7d'], baseline['util_7d'])
        ...
        util_pct_5h = int(snap['util_5h'] * 100 + 1e-9)
        util_pct_7d = int(snap['util_7d'] * 100 + 1e-9)
        next_5h = max(0.0, est_units_per_tick_5h - used_units_since_5h_tick)
        next_7d = max(0.0, est_units_per_tick_7d - used_units_since_7d_tick)

        crossed_5h_str = green(str(crossed_5h)) if crossed_5h > 0 else dim(str(crossed_5h))
        crossed_7d_str = green(str(crossed_7d)) if crossed_7d > 0 else dim(str(crossed_7d))
        total_5h_str = bold(str(ticks_now_5h)) if ticks_now_5h >= need_5h else str(ticks_now_5h)
        total_7d_str = bold(str(ticks_now_7d)) if ticks_now_7d >= need_7d else str(ticks_now_7d)

        log(f"iteration {bold(iter_num)}: prompt size {cyan(size_name)}, took {wall_ms/1000:.1f} seconds")
        log_raw(
            f"  5h window: utilization {yellow(f'{util_pct_5h}%')}, "
            f"ticks crossed this call: {crossed_5h_str}, "
            f"ticks crossed total: {total_5h_str} of {need_5h} needed, "
            f"input-equivalent tokens until next tick: ~{bold(f'{next_5h:,.0f}')}"
        )
        log_raw(
            f"  7d window: utilization {yellow(f'{util_pct_7d}%')}, "
            f"ticks crossed this call: {crossed_7d_str}, "
            f"ticks crossed total: {total_7d_str} of {need_7d} needed, "
            f"input-equivalent tokens until next tick: ~{bold(f'{next_7d:,.0f}')}"
        )
